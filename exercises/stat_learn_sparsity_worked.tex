...
The same notation will be used for subsequent problems involving the group
lasso.

\medskip

The text notes that for the formulation of the group lasso in
(\ref{eq:group_lasso}), since all groups are equally penalized by $ \lambda $,
larger groups, i.e. $ \mathbf{Z}_k $ with more columns, are more likely to be
selected.
...
We return to the intercept-free group lasso problem with $ G \in \mathbb{N} $ groups described in (\ref{eq:group_lasso}). Now, we consider the special case where $ \mathbf{Z}_k^\top\mathbf{Z}_k = \mathbf{I} $,
