...
\begin{figure}
    ...
    \begin{tikzpicture}
        ...
        % }
        %
        % originally, there was the issue of the CMake buld integration not
        % being smart enough to scan the input table files as part of the build
        % dependency graph, but by using the -recorder option with pdfTeX and
        % stripping out duplicate and temporary input files, CMake can now
        % optimally recompile the PDF whenever any PDF input files change.
        %
